...
1. **运行期间请勿移动鼠标或操作电脑**，否则可能干扰脚本执行
2. **首次使用前务必先测试**，建议先设置小范围（如1-5帧）测试是否正常
3. **确保FLIR窗口位置固定**，每次运行时窗口位置应与获取坐标时一致
4. **建议最大化FLIR窗口**，以获得稳定的坐标位置
5. **备份重要数据**，虽然脚本不会修改原始数据，但建议做好备份
